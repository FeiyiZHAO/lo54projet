...
public interface SessionDao extends JpaRepository<CourseSession, Long> {
	...
	@Query(value = "select s.COURSE_CODE,c.TITLE," + "s.START_DATE," + "s.END_DATE," + "s.MAX,"
			...
			+ " group by s.COURSE_CODE,c.TITLE,s.START_DATE,s.END_DATE,l.LOCATION_CITY"
			+ " where cl.COURSE_SESSION_ID = s.COURSE_SESSION_ID and s.LOCATION_ID=?1 and s.START_DATE>?2 and c.TITLE like ?3", nativeQuery = true)
	...
	@Query(value = "select s.COURSE_CODE,c.TITLE," + "s.START_DATE," + "s.END_DATE," + "s.MAX,"
			...
			+ " inner join location l" + " on l.LOCATION_ID=s.LOCATION_ID"
			+ " group by s.COURSE_CODE,c.TITLE,s.START_DATE,s.END_DATE,l.LOCATION_CITY"
			+ " where cl.COURSE_SESSION_ID = s.COURSE_SESSION_ID and s.LOCATION_ID=?1 and c.TITLE like ?2", nativeQuery = true)
	...
	@Query(value = "select s.COURSE_CODE,c.TITLE," + "s.START_DATE," + "s.END_DATE," + "s.MAX,"
			...
			+ " from client cl, course c inner join course_session s" + " on s.COURSE_CODE=c.COURSE_CODE"
			+ " inner join location l" + " on l.LOCATION_ID=s.LOCATION_ID"
			+ " where cl.COURSE_SESSION_ID = s.COURSE_SESSION_ID and c.TITLE like ?1", nativeQuery = true)
	...
	@Query(value = "select s.COURSE_CODE,c.TITLE," + "s.START_DATE," + "s.END_DATE," + "s.MAX,"
			...
			+ " from client cl, course c inner join course_session s" + " on s.COURSE_CODE=c.COURSE_CODE"
			+ " inner join location l" + " on l.LOCATION_ID=s.LOCATION_ID"
			+ " where cl.COURSE_SESSION_ID = s.COURSE_SESSION_ID and s.START_DATE>?1 and c.TITLE like ?2", nativeQuery = true)
