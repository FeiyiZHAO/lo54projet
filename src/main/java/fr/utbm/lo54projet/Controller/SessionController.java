 ...
 * 
	...
	public String getCourse(HttpServletRequest request,@RequestParam("location") String city, Model model) {
		city= request.getParameter("location");
        model.addAttribute("location",city);
        Long id = locationService.findId(city);
        List<CourseSession> list = sessionService.getByLocation(id);
 ...
 * 	
	@RequestMapping("sessions")
	public String getCourse(HttpServletRequest request,@RequestParam("date") String str,Model model) throws ParseException {
		...
		List<CourseSession> list = sessionService.getByDateA(sqlDate);
        model.addAttribute("sessions",list);
		return "sessions";
	}
*/
